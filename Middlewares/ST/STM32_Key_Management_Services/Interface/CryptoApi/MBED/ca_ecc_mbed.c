...
static psa_status_t wrap_import_ecc_pubKey_into_psa(psa_key_handle_t *P_Key_Handle,
                                                    ...
                                                    uint32_t P_KeySize)
{
  psa_status_t psa_ret_status = CA_RSA_ERR_GENERIC;
  ...
  psa_set_key_usage_flags(&key_attributes, P_Psa_Usage);
  psa_set_key_algorithm(&key_attributes, P_Psa_Algorithm);
  psa_set_key_type(&key_attributes, PSA_KEY_TYPE_ECC_PUBLIC_KEY(PSA_ECC_FAMILY_SECP_R1));
  psa_ret_status = psa_import_key(&key_attributes, P_pEcc_pubKey, P_KeySize, P_Key_Handle);
...
static uint8_t bignum_to_mpi(mbedtls_mpi *P_pMpi, BigNum_stt *P_pBigNum)
{
  ...
  P_pMpi->MBEDTLS_PRIVATE(s) = (P_pBigNum->mSignFlag == 0) ? 1 : -1;
  /* Increase mpi size if needed */
  mbed_return = mbedtls_mpi_grow(P_pMpi, P_pBigNum->mNumDigits);
  if (mbed_return == 0)
  {
    P_pMpi->MBEDTLS_PRIVATE(n) = P_pBigNum->mNumDigits;
    /* Copy data from bignum to mpi */
    for (uint32_t i = 0; i < P_pMpi->MBEDTLS_PRIVATE(n); i++)
    {
      P_pMpi->MBEDTLS_PRIVATE(p)[i] = P_pBigNum->pmDigit[i];
    }
    return WRAP_SUCCESS;
  }
  else
...
static uint8_t mpi_to_BigNum(mbedtls_mpi *P_pMpi, BigNum_stt *P_pBigNum)
{
  ...
  if (P_pMpi->MBEDTLS_PRIVATE(n) > P_pBigNum->mSize)
  {
    mbed_return = mbedtls_mpi_shrink(P_pMpi, P_pBigNum->mSize);
  }
  /* If shrink failed, return on error */
  if ((P_pMpi->MBEDTLS_PRIVATE(n) > P_pBigNum->mSize) || (mbed_return != 0))
  {
    return WRAP_FAILURE;
  }
  /* mSignFlag is 0 for positive or 1 for negative,
  and s is 1 for positive and -1 for negative */
  P_pBigNum->mSignFlag = (P_pMpi->MBEDTLS_PRIVATE(s) == 1) ? 0 : 1;
  P_pBigNum->mNumDigits = (uint16_t)(P_pMpi->MBEDTLS_PRIVATE(n));
  /* Copy data from mpi to bignum */
  for (uint32_t i = 0; i < P_pMpi->MBEDTLS_PRIVATE(n); i++)
  {
    P_pBigNum->pmDigit[i] = P_pMpi->MBEDTLS_PRIVATE(p)[i];
  }
    ...
    for (i = 0U; i < size_tp; i++)
    {
      ...
      for (j = 0U; j < 4U; j++)
      {
        ...
        if (size_counter <= Psize)
        {
          ...
          P_pMpi->MBEDTLS_PRIVATE(p)[size_tp - i - 1U] += (uint32_t)(P_pArray[(4U * i) + j]) << (8U * (3U - j));
        }
        else
        {
          /* If there is not enough uint8 data to fill alll the mpi elements, pad with previous uint8 element */
          P_pMpi->MBEDTLS_PRIVATE(p)[size_tp - i - 1U] = P_pMpi->MBEDTLS_PRIVATE(p)[size_tp - i - 1U] >> 8U;
        }
      }
    }
    /*Fill the rest of the structure*/
    P_pMpi->MBEDTLS_PRIVATE(n) = size_tp;
    P_pMpi->MBEDTLS_PRIVATE(s) = 1;
  }
  else
  {
    return WRAP_FAILURE;
                        ...
                        CA_membuf_stt *P_pMemBuf)
{
  ...
  switch (P_pECctx->mAsize)
  {
    case (CA_CRL_ECC_P192_SIZE) :
      mbed_return = mbedtls_ecp_group_load(&peer_ctx.MBEDTLS_PRIVATE(grp), MBEDTLS_ECP_DP_SECP192R1);
      break;

    case (CA_CRL_ECC_P256_SIZE) :
      mbed_return = mbedtls_ecp_group_load(&peer_ctx.MBEDTLS_PRIVATE(grp), MBEDTLS_ECP_DP_SECP256R1);
      break;

    case (CA_CRL_ECC_P384_SIZE) :
      mbed_return = mbedtls_ecp_group_load(&peer_ctx.MBEDTLS_PRIVATE(grp), MBEDTLS_ECP_DP_SECP384R1);
      break;
  ...
  if (0 != mbedtls_ecp_point_read_binary(&peer_ctx.MBEDTLS_PRIVATE(grp), &dev_eph_pub_key_ec_point,
  ...
  if (0 != mbedtls_ecp_check_pubkey(&peer_ctx.MBEDTLS_PRIVATE(grp), &dev_eph_pub_key_ec_point))
  {
    /* Upon error, release context and point */
    mbedtls_ecp_point_free(&dev_eph_pub_key_ec_point);
    mbedtls_ecdsa_free(&peer_ctx);
  ...
  if (0 != mbedtls_mpi_read_binary(&peer_ctx.MBEDTLS_PRIVATE(d), pTmp, (uint32_t)PeerPrvKeyEcPointLen))
  {
    /* Upon error, release context and point */
    mbedtls_ecp_point_free(&dev_eph_pub_key_ec_point);
    mbedtls_ecdsa_free(&peer_ctx);
  ...
  if (0 != mbedtls_ecp_check_privkey(&peer_ctx.MBEDTLS_PRIVATE(grp), &peer_ctx.MBEDTLS_PRIVATE(d)))
  {
    /* Upon error, release context and point */
    mbedtls_ecp_point_free(&dev_eph_pub_key_ec_point);
    mbedtls_ecdsa_free(&peer_ctx);
  ...
  mbed_return = mbedtls_ecp_mul(&peer_ctx.MBEDTLS_PRIVATE(grp), &peer_shared_secret_x_ec_point, &peer_ctx.MBEDTLS_PRIVATE(d),
                                &dev_eph_pub_key_ec_point, mbedtls_psa_get_random, NULL);
  ...
  else
  {
    ...
    wrap_status = mpi_to_BigNum(&(peer_shared_secret_x_ec_point.MBEDTLS_PRIVATE(X)), P_pECresultPnt->pmX);
    wrap_status |= mpi_to_BigNum(&(peer_shared_secret_x_ec_point.MBEDTLS_PRIVATE(Y)), P_pECresultPnt->pmY);
    wrap_status |= mpi_to_BigNum(&(peer_shared_secret_x_ec_point.MBEDTLS_PRIVATE(Z)), P_pECresultPnt->pmZ);
  }
                       ...
                       CA_membuf_stt *P_pMemBuf)
{
  ...
  psa_ecc_family_t wrap_ecc_curve = {0};
  ...
  {
    /* Verify */
    psa_ret_status = psa_verify_hash(ECDSAkeyHandle,
                                     psa_algorithm,
                                     P_pDigest,
                     ...
                     CA_membuf_stt *P_pMemBuf)
{
  ...
  /* Generate keys */
  if (0 != mbedtls_ecdsa_genkey(&peer_ctx, peer_ctx.MBEDTLS_PRIVATE(grp).id, mbedtls_entropy_func, &_entropy))
  {
    /* Upon error, release contexts */
    mbedtls_ctr_drbg_free(&_ctr_drbg);
    mbedtls_entropy_free(&_entropy);
  ...
  /* Copy secret key */
  wrap_status = mpi_to_BigNum(&(peer_ctx.MBEDTLS_PRIVATE(d)), P_pPrivKey->pmD);

  /* Copy public key */
  wrap_status |= mpi_to_BigNum(&(peer_ctx.MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(X)), P_pPubKey->pmX);
  wrap_status |= mpi_to_BigNum(&(peer_ctx.MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Y)), P_pPubKey->pmY);
  wrap_status |= mpi_to_BigNum(&(peer_ctx.MBEDTLS_PRIVATE(Q).MBEDTLS_PRIVATE(Z)), P_pPubKey->pmZ);
