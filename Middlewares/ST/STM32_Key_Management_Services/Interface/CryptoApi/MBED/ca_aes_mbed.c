                                                     ...
                                                     uint32_t P_KeySize)
{
  ...
  psa_set_key_bits(&key_attributes, 8U*P_KeySize);
  psa_ret_status = psa_import_key(&key_attributes, P_pAes_Key, P_KeySize, P_Key_Handle);
                                  ...
                                  int32_t *P_pOutputSize)
{
  ...
  if (psa_ret_status == PSA_SUCCESS)
  {
    ...
    wrap_iv_psa_to_crypto(P_pAESCBCctx->cipher_op.MBEDTLS_PRIVATE(ctx).mbedtls_ctx.MBEDTLS_PRIVATE(ctx).MBEDTLS_PRIVATE(cipher).MBEDTLS_PRIVATE(iv), P_pAESCBCctx->amIv);
  }
  else
  {
                                  ...
                                  int32_t       *P_pOutputSize)
{
  ...
  {
    /*Update of the output context with the new IV*/
    wrap_iv_psa_to_crypto(P_pAESCBCctx->cipher_op.MBEDTLS_PRIVATE(ctx).mbedtls_ctx.MBEDTLS_PRIVATE(ctx).MBEDTLS_PRIVATE(cipher).MBEDTLS_PRIVATE(iv),
                          P_pAESCBCctx->amIv);
  }
  else
  {
                                  ...
                                  int32_t       *P_pOutputSize)
{
  ...
  mbedtls_status = mbedtls_gcm_finish(&P_pAESGCMctx->mbedtls_ctx, NULL, 0U, &olen,
                                      P_pOutputBuffer,
                                      (size_t) P_pAESGCMctx->mTagSize);
  if (mbedtls_status == MBEDTLS_SUCCESS)
  {
