...
static psa_status_t wrap_import_der_Key_into_psa(psa_key_handle_t *P_Key_Handle,
                                                 ...
                                                 uint32_t P_KeySize, uint32_t KeyBytes)
{
  ...
  psa_set_key_type(&key_attributes, P_Psa_key_type);
  psa_set_key_bits(&key_attributes, 8U*KeyBytes);
  psa_ret_status = psa_import_key(&key_attributes, P_pDer_pubKey, P_KeySize, P_Key_Handle);
                             ...
                             CA_membuf_stt *P_pMemBuf)
{
  ...
  if (wrap_ret_status == WRAP_SUCCESS)
  {
    ...
    psa_ret_status = wrap_import_der_Key_into_psa(&psa_key_handle,
                                                  ...
                                                  length, P_pPrivKey->mModulusSize);
    if (psa_ret_status == PSA_SUCCESS)
    {
      /* Sign */
      psa_ret_status = psa_sign_message(psa_key_handle,
                                        psa_algorithm,
                                        P_pDigest,
                                        wrap_hash_size,
                                        P_pSignature,
                               ...
                               CA_membuf_stt *P_pMemBuf)
{
  ...
  if (wrap_ret_status == WRAP_SUCCESS)
  {
    ...
    psa_ret_status = wrap_import_der_Key_into_psa(&psa_key_handle,
                                                  ...
                                                  DER_pubKey,
                                                  pwrap_der_size, P_pPubKey->mModulusSize);
    if (psa_ret_status == CA_RSA_SUCCESS)
    {
      /* Verify */
      psa_ret_status = psa_verify_message(psa_key_handle,
                                          psa_algorithm,
                                          P_pDigest,
                                          wrap_hash_size,
                                          P_pSignature,
