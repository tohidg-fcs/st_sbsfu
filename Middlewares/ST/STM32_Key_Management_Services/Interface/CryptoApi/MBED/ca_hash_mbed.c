...
int32_t CA_SHA1_Init(CA_SHA1ctx_stt *P_pSHA1ctx)
{
  ...
  if (mbedtls_sha1_starts(&(P_pSHA1ctx->hash_ctx)) != 0)
  {
                       ...
                       int32_t P_inputSize)
{
  ...
  if (mbedtls_sha1_update(&(P_pSHA1ctx->hash_ctx), P_pInputBuffer, (size_t)P_inputSize) != 0)
  {
                       ...
                       int32_t *P_pOutputSize)
{
  ...
  if (mbedtls_sha1_finish(&(P_pSHA1ctx->hash_ctx), P_pOutputBuffer) != 0)
  {
...
int32_t CA_SHA256_Init(CA_SHA256ctx_stt *P_pSHA256ctx)
{
  ...
  if (mbedtls_sha256_starts(&(P_pSHA256ctx->hash_ctx), 0 /* is224 = 0 for SHA256 */) != 0)
  {
    hash_ret_status = CA_HASH_ERR_BAD_PARAMETER;
  }
  else
                         ...
                         int32_t P_inputSize)
{
  ...
  if (mbedtls_sha256_update(&(P_pSHA256ctx->hash_ctx), P_pInputBuffer, (size_t)P_inputSize) != 0)
  {
    hash_ret_status = CA_HASH_ERR_BAD_OPERATION;
  }
  else
                         ...
                         int32_t *P_pOutputSize)
{
  ...
  if (mbedtls_sha256_finish(&(P_pSHA256ctx->hash_ctx), P_pOutputBuffer) != 0)
  {
    hash_ret_status = CA_HASH_ERR_BAD_CONTEXT;
  }
  else
