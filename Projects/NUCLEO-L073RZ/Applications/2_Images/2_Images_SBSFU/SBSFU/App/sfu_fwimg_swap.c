...
/* The size for progressive copy check has to be a erase page size to be able to be erased independently */
#define TRAILER_SIZE(A) (FLASH_PAGE_SIZE + (uint32_t)(TRAILER_HEADER))
...
/* Default flash value is 0x00 and not 0xFF */
static const int32_t SWAPPED[sizeof(SFU_LL_FLASH_write_t)]  = {0x55555555};
static const int32_t NOT_SWAPPED[sizeof(SFU_LL_FLASH_write_t)] = {0};
...
static SFU_ErrorStatus CheckTrailerMagic(uint32_t DwlSlot)
{
  ...
  (void) memset(erased_flash_pattern, 0x00, MAGIC_LENGTH);
...
static SFU_ErrorStatus EraseSlotIndex(uint32_t SlotNumber, uint32_t index)
{
  ...
  if ((SlotNumber >= SLOT_ACTIVE_1) && (SlotNumber <= (SLOT_ACTIVE_1 + SFU_NB_MAX_ACTIVE_IMAGE)) && (index == 0U))
  {
     ...
     *      Secure Engine
     *  Thus the the erasing is split in 2 actions (HEADER + remaining part)
     */
        ...
        if (index_active_slot == index_active_slot_final_end)
        {
          ...
          if ((CHUNK_0_ADDR(ActiveSlot, index_active_slot, chunk) <
              ...
              (CHUNK_0_ADDR(ActiveSlot, index_active_slot, (chunk + 1)) >
               (CHUNK_0_ADDR(ActiveSlot, index_active_slot_final_end, 0) + offset_block_final_end)))
          {
            memset(buffer + (offset_block_final_end % SFU_IMG_CHUNK_SIZE), 0x00,
        ...
        if (e_ret_status == SFU_SUCCESS)
        {
          ...
          if ((size & ((uint32_t)sizeof(SFU_LL_FLASH_write_t) - 1U)) != 0U)
          {
            ...
            * By construction, SFU_IMG_CHUNK_SIZE is a multiple of sizeof(SFU_LL_FLASH_write_t) so there is no risk to
            ...
            while (oldsize < size)
            {
              fw_decrypted_chunk[oldsize] = 0x00;
              oldsize++;
            }
          }
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CheckSwapImageHandling(void)
{
   ...
   */
  if (((SlotStartAdd[SLOT_SWAP] - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
  {
