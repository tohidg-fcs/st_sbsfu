...
#endif /* (__GNUC__) */
    {
      if (((SlotStartAdd[SLOT_ACTIVE_1 + i] - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
      {
...
#endif /* (__GNUC__) */
    {
      if (((SlotStartAdd[SLOT_DWL_1 + i] - FLASH_BASE) / (FLASH_PAGE_SIZE * 32U)) <= SFU_PROTECT_WRP_PAGE_END_1)
      {
...
#endif /* (__GNUC__) */
    {
      if (!((SlotHeaderAdd[SLOT_ACTIVE_1 + i] >= SFU_PROTECT_FWALL_NVDATA_ADDR_START)
            && ((SlotHeaderAdd[SLOT_ACTIVE_1 + i] + SE_FW_HEADER_TOT_LEN) <= (SFU_PROTECT_FWALL_NVDATA_ADDR_START +
                                                                              SFU_PROTECT_FWALL_NVDATA_SIZE))))
      {
...
SFU_ErrorStatus SFU_IMG_InvalidateCurrentFirmware(uint32_t SlotNumber)
{
  ...
  e_ret_status = SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *)(SlotStartAdd[SlotNumber] + SFU_IMG_IMAGE_OFFSET),
                                         SLOT_SIZE(SlotNumber) - SFU_IMG_IMAGE_OFFSET);
