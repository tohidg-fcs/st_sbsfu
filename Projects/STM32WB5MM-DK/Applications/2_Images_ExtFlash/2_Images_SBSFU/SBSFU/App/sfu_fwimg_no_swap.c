...
/**
  ...
  * @retval SFU_ErrorStatus SFU_SUCCESS if successful, SFU_ERROR otherwise.
  */
static SFU_ErrorStatus CopyFw(uint32_t SlotOrigine, uint32_t SlotDestination, uint32_t Length)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  ...
  /* Destination slot size should be at least >= origine slot size */
  if (SLOT_SIZE(SlotDestination) <  SLOT_SIZE(SlotOrigine))
  {
    return SFU_ERROR;
  }
