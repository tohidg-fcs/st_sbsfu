...
HAL_StatusTypeDef LOADER_LL_FLASH_EXT_Write(uint8_t  *pDestination, const uint8_t *pSource, uint32_t Length)
{
  ...
  if (BSP_QSPI_Write(0, (uint8_t *) pSource, destination - EXTERNAL_FLASH_ADDRESS, Length) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
...
HAL_StatusTypeDef LOADER_LL_FLASH_EXT_Read(uint8_t *pDestination, const uint8_t *pSource, uint32_t Length)
{
  ...
  if (BSP_QSPI_Read(0, (uint8_t *) pDestination, source - EXTERNAL_FLASH_ADDRESS, Length) != BSP_ERROR_NONE)
  {
    return HAL_ERROR;
  }
  else
