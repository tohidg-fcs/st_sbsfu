...
uint32_t FD_EraseSectors(uint32_t FirstSector, uint32_t NbrOfSectors)
{
  ...
  else
  {
     ...
     *  On reception of this command, the CPU2 will disable the BLE timing protection versus flash erase processing
...
static SingleFlashOperationStatus_t ProcessSingleFlashOperation(FlashOperationType_t FlashOperationType,
                                                                ...
                                                                uint64_t Data)
{
  SemStatus_t cpu1_sem_status = SEM_LOCK_SUCCESSFUL;
  SemStatus_t cpu2_sem_status = SEM_LOCK_SUCCESSFUL;
  ...
  do
  {
    ...
    if(cpu1_sem_status == SEM_LOCK_SUCCESSFUL)
    {
      ...
      if(cpu2_sem_status == SEM_LOCK_SUCCESSFUL)
      {
         ...
         */
#if defined(OTA_SBSFU) && defined(SFU_IWDG_PROTECT_ENABLE)
...
WaitedSemStatus_t FD_WaitForSemAvailable(WaitedSemId_t WaitedSemId)
{
  /**
   *
   * Return WAITED_SEM_FREE in order to keep looping in the driver until the action is executed.
   *
   */
