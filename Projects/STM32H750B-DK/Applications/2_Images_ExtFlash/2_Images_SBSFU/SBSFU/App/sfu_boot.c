        ...
        if (SFU_SUCCESS == e_ret_status)
        {
          ...
          if (SFU_SUCCESS == e_ret_status)
          {
            ...
            else
            {
              ...
              e_ret_status = SFU_IMG_SecureCounterRead(SLOT_ACTIVE_1 + i, &secure_version);
              if (SFU_SUCCESS == e_ret_status)
              {
                 e_ret_status = (secure_version == fw_image_header_validated.FwVersion) ? SFU_SUCCESS : SFU_ERROR;
              }
#if defined(SFU_VERBOSE_DEBUG_MODE)
              if (SFU_SUCCESS != e_ret_status)
              {
                TRACE("\r\n\t  NV Counter not equal to FW header version in slot SLOT_ACTIVE_%d", SLOT_ACTIVE_1 + i);
              }
#endif /* SFU_VERBOSE_DEBUG_MODE */
            }
