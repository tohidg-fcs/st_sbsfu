...
SFU_IMG_InitStatusTypeDef SFU_IMG_InitImageHandling(void)
{
...
#endif /* (__GNUC__) */
    {
      if (((SlotStartAdd[SLOT_DWL_1 + i] - FLASH_BASE) / FLASH_SECTOR_SIZE) <= SFU_PROTECT_WRP_PAGE_END_1)
      {
  ...
  /*
   * Sanity check: let's make sure the Anti-Rollback secure counter and its area is valid)
   */
  if (SFU_IMG_SecureCounterInit() != SFU_SUCCESS )
  {
    TRACE("\r\n= [FWIMG] Anti-Rollback secure counter initialization failure\r\n");
    e_ret_status = SFU_IMG_INIT_SECURE_COUNTER_ERROR;
  }
...
#if defined(SFU_NV_SECURE_COUNTERS)
  /* If a version secure counter has been implemented, update it */
  if (e_ret_status == SFU_SUCCESS)
  {
    e_ret_status = SFU_IMG_SecureCounterWrite(SlotNumber, pFWImageHeader->FwVersion );
  }
#endif /* SFU_NV_SECURE_COUNTERS */
...
SFU_ErrorStatus SFU_IMG_CheckFwVersion(uint32_t ActiveSlot, uint16_t CurrentVersion, uint16_t CandidateVersion)
{
  ...
  /* If a version with secure counter has been implemented, rely on it */
  if (SFU_IMG_SecureCounterRead(ActiveSlot, &secure_version) == SFU_SUCCESS)
  {
    CurrentVersion = (uint16_t) secure_version;
  }
  else
  {
    CurrentVersion = 0xFFFF; /* Invalid value to invalidate the next check */
  }
#else
  UNUSED(ActiveSlot);
#endif /* SFU_NV_SECURE_COUNTERS */
