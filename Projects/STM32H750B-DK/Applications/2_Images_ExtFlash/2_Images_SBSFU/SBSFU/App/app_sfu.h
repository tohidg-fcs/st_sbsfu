...
/**
  ...
  *
  */
#if ( (SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITHOUT_ENCRYPT_SHA256) ||((SECBOOT_CRYPTO_SCHEME == SECBOOT_ECCDSA_WITH_AES128_CTR_SHA256)) )
...
/* Secure counter management for anti-rollback control during FW update process :
   - Non volatile secure counters are located inside internal flash
   - H750 is a single sector flash
   - WRP granularity is one sector i.e. all the 128 Kbytes internal flash
   ==> this feature cannot be activated with WRP security activated !!!
*/
#define SFU_NV_SECURE_COUNTERS
