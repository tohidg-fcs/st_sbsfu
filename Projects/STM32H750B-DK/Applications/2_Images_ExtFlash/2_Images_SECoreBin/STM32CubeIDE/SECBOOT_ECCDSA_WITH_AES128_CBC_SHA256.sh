...
header_userapp=$userAppBinary"/Header_"$execname".bin"
...
if [ $ret -eq 0 ]; then
  ...
  if [ $ret -eq 0 ]; then 
    ...
    if [ $ret -eq 0 ]; then
      ...
      ret=$?
      if [ $ret -eq 0 ]; then
        command=$cmd" "$prepareimage" extract -d __ICFEDIT_SLOT_Active_"$fwid"_header__ "$mapping
        header=`$command`
        ret=$?
        echo "header $header"
        echo "header $header" >> $projectdir"/output.txt"
        let "offset_bin=$header+$offset"
        if [ $ret -eq 0 ]; then
          command=$cmd" "$prepareimage" mergev2 -v 0 -b "$headerbin"@"$header";"$bin"@"$offset_bin" "$header_userapp
          $command >> $projectdir"/output.txt"
          ret=$?
