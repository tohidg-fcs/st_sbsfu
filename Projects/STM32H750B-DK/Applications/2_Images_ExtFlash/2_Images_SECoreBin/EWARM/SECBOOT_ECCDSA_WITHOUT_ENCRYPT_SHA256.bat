...
set "header_userapp=%userAppBinary%\\Header_%execname%.bin"
...
set "sbsfuelf=%SBSFUBootLoader%\\2_Images_SBSFU\\EWARM\\STM32H750B_DISCO\\Exe\\STM32H750B_DISCO.out"
...
set /A offset_bin=%header%+%offset%

set "command=%python%%prepareimage% mergev2 -b %headerbin%@%header%;%bin%@%offset_bin% %header_userapp% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
