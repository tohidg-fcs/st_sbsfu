...
/**
  ...
  * @note              when calling the function, caller provides size of the
                                                    ...
                                                    uint8_t *p_CertChain_RootCA)
{
  ...
  else
  {
    ...
    /* Double the security checks on the certificate and the signature before
       saving the PreVerifiedHash, as it will be used to bypass further security
       checks. */
    ret = SFU_SCHEME_X509_CRT_VerifyCert(FW_Int_Certs, p_CertChain_OEM, p_CertChain_RootCA, &MbedCertChain);
    if (SFU_SUCCESS != ret)
    {
    ...
    /* Store the PreVerifiedHash so that further security checks of this image
       can be bypassed for performance reasons. */
