...
:: arg5 is the version
:: arg6 when present forces "bigelf" generatio
@echo off
set "projectdir=%1"
set "execname=%~n3"
set "elf=%2"
...
set "sbsfuelf=%SBSFUBootLoader%\\2_Images_SBSFU\\MDK-ARM\\B-L4S5I-IOT01_2_Images_SBSFU\\SBSFU.axf"
set "fw_sign_ecckey=%SBSFUBootLoader%\\2_Images_SECoreBin\\Binary\\sbsfu_fwsign_oem%fwid%_prv.pem"
set "cert_fw_leaf=%SBSFUBootLoader%\\2_Images_SECoreBin\\Binary\\sbsfu_fwsign_oem%fwid%_crt.der"
set "cert_fw_inter=%SBSFUBootLoader%\\2_Images_SECoreBin\\Binary\\sbsfu_inter2_oem%fwid%_crt.der"
...
set "command=%python%%prepareimage% pack -m %magic% -k %fw_sign_ecckey% -p 1 -r 44 -v %version% --cert_fw_inter %cert_fw_inter% --cert_fw_leaf %cert_fw_leaf% -f %bin% -t %sign% %sfb% -o %offset% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% header -m %magic% -k %fw_sign_ecckey% -p 1 -r 44 -v %version% -f %bin% -t %sign% -o %offset% --cert_fw_inter %cert_fw_inter% --cert_fw_leaf %cert_fw_leaf% %headerbin% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
set "command=%python%%prepareimage% pack -m %magic% -k %fw_sign_ecckey% -p 1 -r 44 -v %version% --cert_fw_inter %cert_fw_inter% --cert_fw_leaf %cert_fw_leaf% -f %bin% -t %sign% -o %offset% --pfw %partialbin% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
