...
if "%crypto%"=="SECBOOT_X509_ECDSA_WITHOUT_ENCRYPT_SHA256" (
  goto X509_ECDSA_WITHOUT_ENCRYPT_SHA256
)
goto end
...
set "rootca=%1\\..\\Binary\\sbsfu_r0_crt.der"
set "command=copy %KMS_config% %KMS_tmp%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% inject -c %rootca% -f %KMS_tmp% -p @SBSFU_ROOT_CA@ %KMS_config%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "inter1ca=%1\\..\\Binary\\sbsfu_inter1_oem1_crt.der"
set "command=copy %KMS_config% %KMS_tmp%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% inject -c %inter1ca% -f %KMS_tmp% -p @SBSFU_OEM_CA_1@ %KMS_config%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "inter1ca=%1\\..\\Binary\\sbsfu_inter1_oem2_crt.der"
IF NOT EXIST %inter1ca% goto :end
set "command=copy %KMS_config% %KMS_tmp%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% inject -c %inter1ca% -f %KMS_tmp% -p @SBSFU_OEM_CA_2@ %KMS_config%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "inter1ca=%1\\..\\Binary\\sbsfu_inter1_oem3_crt.der"
IF NOT EXIST %inter1ca% goto :end
set "command=copy %KMS_config% %KMS_tmp%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% inject -c %inter1ca% -f %KMS_tmp% -p @SBSFU_OEM_CA_3@ %KMS_config%"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
IF %ERRORLEVEL% NEQ 0 goto :error
goto end
