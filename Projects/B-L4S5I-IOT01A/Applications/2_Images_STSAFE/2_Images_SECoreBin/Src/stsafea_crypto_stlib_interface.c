/**
  ******************************************************************************
  * @file    stsafea_crypto_stlib_interface.c
  * @author  SMD/AME application teams
  * @brief   Crypto Interface file to support the crypto services required by the
  *          STSAFE-A Middleware and offered by the STM32 crypto library:
  *           + Key Management
  *           + SHA
  *           + AES
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2016 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */

/* Includes ------------------------------------------------------------------*/
#include <stdlib.h>
#include <string.h>
#include "CryptoAPI/ca.h"
#include "stsafea_crypto.h"
#include "stsafea_interface_conf.h"
#include MCU_PLATFORM_INCLUDE
#include "se_key.h"

/* Private typedef -----------------------------------------------------------*/

/* Private defines -----------------------------------------------------------*/

/* Private macros ------------------------------------------------------------*/

/* Private variables ---------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
#if (USE_PRE_LOADED_HOST_KEYS)
uint8_t  aHostCipherKey[STSAFEA_HOST_KEY_LENGTH] __attribute__((aligned(4))); /*!< STSAFE-A's Host cipher key */
uint8_t  aHostMacKey   [STSAFEA_HOST_KEY_LENGTH] __attribute__((aligned(4))); /*!< STSAFE-A's Host Mac key */
#else
/*!< STSAFE-A's Host cipher key */
uint8_t  aHostCipherKey[] =
{
  0x11, 0x11, 0x22, 0x22, 0x33, 0x33, 0x44, 0x44, 0x55, 0x55, 0x66, 0x66, 0x77, 0x77, 0x88, 0x88
};
/*!< STSAFE-A's Host Mac key */
uint8_t  aHostMacKey   [] =
{
  0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF
};
#endif /* USE_PRE_LOADED_HOST_KEYS */


/* Global variables ----------------------------------------------------------*/
#ifdef CA_ROUTE_AES_CMAC
static CA_AESCMACctx_stt  CMacCtxStt;
#endif /*CA_ROUTE_AES_CMAC*/

/* Private function prototypes -----------------------------------------------*/

/* Functions Definition ------------------------------------------------------*/

/** @addtogroup CRYPTO_IF_Exported_Functions_Group1 Host MAC and Cipher keys Initialization
  *  @brief    Crypto Interface APIs to be implemented at application level. Templates are provided.
  *
@verbatim
 ===============================================================================
           ##### Host MAC and Cipher keys Initialization functions #####
 ===============================================================================

@endverbatim
  * @{
  */

/**
  * @brief   StSafeA_HostKeys_Init
  *          Initialize STSAFE-Axxx Host MAC and Cipher Keys that will be used by the crypto interface layer
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   None
  * @retval  0 if success. An error code otherwise
  */
int32_t StSafeA_HostKeys_Init()
{
#if (USE_PRE_LOADED_HOST_KEYS)
  /* Keys stored in the MCU Flash are loaded into the SRAM.
     SE_ReadKey_Pairing retrieves an array containing the concatenation of STSAFE Mac and Cipher keys*/

  uint8_t host_mac_cipher_key_tmp[2U * STSAFEA_HOST_KEY_LENGTH];

  SE_ReadKey_Pairing(&host_mac_cipher_key_tmp[0U]);

  /* Set and keep the keys that will be used during the Crypto / MAC operations */
  /* Host MAC Key */
    
  /* Accessing confidential array values shall not be done in a zero-offset
    sequential manner (e.g. from left to right). Instead, choose (dynamically)
    an offset and traverse the array starting at that offset modulo the length of the array.*/
   
  for (int i = 0, j = (rand() & (STSAFEA_HOST_KEY_LENGTH-1)); i < STSAFEA_HOST_KEY_LENGTH; i++, j = ((j+1) 
                                                                           & (STSAFEA_HOST_KEY_LENGTH-1)))
    aHostMacKey[j] = host_mac_cipher_key_tmp[j]; /* start at a random index in the range 0..STSAFEA_HOST_KEY_LENGTH-1 */

  /* Host Cipher Key */
  
  for (int i = 0, j = (rand() & (STSAFEA_HOST_KEY_LENGTH-1)); i < STSAFEA_HOST_KEY_LENGTH; i++, j = ((j+1) 
                                                                            & (STSAFEA_HOST_KEY_LENGTH-1)))
    aHostCipherKey[j] = host_mac_cipher_key_tmp[j+STSAFEA_HOST_KEY_LENGTH]; 
  
  /* Clear host_mac_cipher_key_tmp array after usage */ 
  (void)memset(host_mac_cipher_key_tmp, 0, sizeof(host_mac_cipher_key_tmp));  
  
#endif /* USE_PRE_LOADED_HOST_KEYS */

  return 0;
}
/**
  * @}
  */


#if (USE_SIGNATURE_SESSION)
/** @addtogroup CRYPTO_IF_Exported_Functions_Group2 HASH Functions
  *  @brief    Crypto Interface APIs to be implemented at application level. Templates are provided.
  *
@verbatim
 ===============================================================================
                          ##### HASH functions #####
 ===============================================================================

@endverbatim
  * @{
  */

/**
  * @brief   StSafeA_SHA_Init
  *          SHA initialization function to initialize the SHA context
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   InHashType : type of SHA
  *          This parameter can be one of the StSafeA_HashTypes_t enum values:
  *            @arg STSAFEA_SHA_256: 256-bits
  *            @arg STSAFEA_SHA_384: 384-bits
  * @param   ppShaCtx : SHA context to be initialized
  * @retval  None
  */
void StSafeA_SHA_Init(StSafeA_HashTypes_t InHashType, void **ppShaCtx)
{
#ifdef CA_ROUTE_HASH_SHA256
  static CA_SHA256ctx_stt  SHA256_ctx_stt;
#endif /* CA_ROUTE_HASH_SHA256 */
#ifdef CA_ROUTE_HASH_SHA384
  static CA_SHA384ctx_stt  SHA384_ctx_stt;
#endif /* CA_ROUTE_HASH_SHA384 */


  switch (InHashType)
  {
#ifdef CA_ROUTE_HASH_SHA256
    case STSAFEA_SHA_256:
      *ppShaCtx = &SHA256_ctx_stt;
      ((SHA256ctx_stt *)*ppShaCtx)->mTagSize = CA_CRL_SHA256_SIZE;
      ((SHA256ctx_stt *)*ppShaCtx)->mFlags = CA_E_HASH_DEFAULT;
      (void)SHA256_Init(*ppShaCtx);
      break;
#endif /* CA_ROUTE_HASH_SHA256 */

#ifdef CA_ROUTE_HASH_SHA384
    case STSAFEA_SHA_384:
      *ppShaCtx = &SHA384_ctx_stt;
      ((SHA384ctx_stt *)*ppShaCtx)->mTagSize = CA_CRL_SHA384_SIZE;
      ((SHA384ctx_stt *)*ppShaCtx)->mFlags = CA_E_HASH_DEFAULT;
      (void)SHA384_Init(*ppShaCtx);
      break;
#endif /* CA_ROUTE_HASH_SHA384 */

    default:
      break;
  }
}


/**
  * @brief   StSafeA_SHA_Update
  *          SHA update function to process SHA over a message data buffer.
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   InHashType : type of SHA
  *          This parameter can be one of the StSafeA_HashTypes_t enum values:
  *            @arg STSAFEA_SHA_256: 256-bits
  *            @arg STSAFEA_SHA_384: 384-bits
  * @param   pShaCtx : SHA context
  * @param   pInMessage : message data buffer
  * @param   InMessageLength : message data buffer length
  * @retval  None
  */
void StSafeA_SHA_Update(StSafeA_HashTypes_t InHashType, void *pShaCtx, uint8_t *pInMessage, uint32_t InMessageLength)
{
  switch (InHashType)
  {
#ifdef CA_ROUTE_HASH_SHA256
    case STSAFEA_SHA_256:
      if (pShaCtx != NULL)
      {
        (void)CA_SHA256_Append(pShaCtx, pInMessage, (int32_t)InMessageLength);
      }
      break;
#endif /* CA_ROUTE_HASH_SHA256 */

#ifdef CA_ROUTE_HASH_SHA384
    case STSAFEA_SHA_384:
      if (pShaCtx != NULL)
      {
        (void)CA_SHA384_Append(pShaCtx, pInMessage, (int32_t)InMessageLength);
      }
      break;
#endif /* CA_ROUTE_HASH_SHA384 */

    default:
      break;
  }
}


/**
  * @brief   StSafeA_SHA_Final
  *          SHA final function to finalize the SHA Digest
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   InHashType : type of SHA
  *          This parameter can be one of the StSafeA_HashTypes_t enum values:
  *            @arg STSAFEA_SHA_256: 256-bits
  *            @arg STSAFEA_SHA_384: 384-bits
  * @param   ppShaCtx : SHA context to be finalized
  * @param   pMessageDigest : message digest data buffer
  * @retval  None
  */
void StSafeA_SHA_Final(StSafeA_HashTypes_t InHashType, void **ppShaCtx, uint8_t *pMessageDigest)
{
  int32_t digest_length;

  switch (InHashType)
  {
#ifdef CA_ROUTE_HASH_SHA256
    case STSAFEA_SHA_256:
      if (*ppShaCtx != NULL)
      {
        (void)CA_SHA256_Finish(*ppShaCtx, pMessageDigest, &digest_length);
        *ppShaCtx = NULL;
      }
      break;
#endif /* CA_ROUTE_HASH_SHA256 */

#ifdef CA_ROUTE_HASH_SHA384
    case STSAFEA_SHA_384:
      if (*ppShaCtx != NULL)
      {
        (void)CA_SHA256_Finish(*ppShaCtx, pMessageDigest, &digest_length);
        *ppShaCtx = NULL;
      }
      break;
#endif /* CA_ROUTE_HASH_SHA384 */

    default:
      break;
  }
}
#endif /* USE_SIGNATURE_SESSION */


/**
  * @}
  */

/** @addtogroup CRYPTO_IF_Exported_Functions_Group3 AES Functions
  *  @brief    Crypto Interface APIs to be implemented at application level. Templates are provided.
  *
@verbatim
 ===============================================================================
                          ##### AES functions #####
 ===============================================================================

@endverbatim
  * @{
  */

/**
  * @brief   StSafeA_AES_MAC_Start
  *          Start AES MAC computation
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   ppAesMacCtx : AES MAC context
  * @retval  None
  */
void StSafeA_AES_MAC_Start(void **ppAesMacCtx)
{
#ifdef CA_ROUTE_AES_CMAC
  *ppAesMacCtx = &CMacCtxStt;

  ((CA_AESCMACctx_stt *)*ppAesMacCtx)->mFlags = CA_E_SK_DEFAULT;
  ((CA_AESCMACctx_stt *)*ppAesMacCtx)->pmKey = aHostMacKey;
  ((CA_AESCMACctx_stt *)*ppAesMacCtx)->mKeySize = 16;
  ((CA_AESCMACctx_stt *)*ppAesMacCtx)->mTagSize = 16;
  (void)CA_AES_CMAC_Encrypt_Init((CA_AESCMACctx_stt *)*ppAesMacCtx);
#endif /* CA_ROUTE_AES_CMAC */
}


/**
  * @brief   StSafeA_AES_MAC_Update
  *          Update / Add data to MAC computation
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pInData : data buffer
  * @param   InDataLength : data buffer length
  * @param   pAesMacCtx : AES MAC context
  * @retval  None
  */
void StSafeA_AES_MAC_Update(uint8_t *pInData, uint16_t InDataLength, void *pAesMacCtx)
{
#ifdef CA_ROUTE_AES_CMAC
  if (pAesMacCtx != NULL)
  {
    (void)CA_AES_CMAC_Encrypt_Append(pAesMacCtx, pInData, (int32_t)InDataLength);
  }
#endif /* CA_ROUTE_AES_CMAC */
}


/**
  * @brief   StSafeA_AES_MAC_LastUpdate
  *          Update / Add data to MAC computation
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pInData : data buffer
  * @param   InDataLength : data buffer length
  * @param   pAesMacCtx : AES MAC context
  * @retval  None
  */
void StSafeA_AES_MAC_LastUpdate(uint8_t *pInData, uint16_t InDataLength, void *pAesMacCtx)
{
#ifdef CA_ROUTE_AES_CMAC
  if (pAesMacCtx != NULL)
  {
    ((CA_AESCMACctx_stt *)pAesMacCtx)->mFlags |= CA_E_SK_FINAL_APPEND;
    (void)CA_AES_CMAC_Encrypt_Append(pAesMacCtx, pInData, (int32_t)InDataLength);
  }
#endif /* CA_ROUTE_AES_CMAC */
}

/**
  * @brief   StSafeA_AES_MAC_Final
  *          Finalize AES MAC computation
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pOutMac : calculated MAC
  * @param   ppAesMacCtx : AES MAC context
  * @retval  None
  */
void StSafeA_AES_MAC_Final(uint8_t *pOutMac, void **ppAesMacCtx)
{
#ifdef CA_ROUTE_AES_CMAC
  int32_t output_length = 0;
  if (*ppAesMacCtx != NULL)
  {
    (void)CA_AES_CMAC_Encrypt_Finish(*ppAesMacCtx, pOutMac, &output_length);
    *ppAesMacCtx = NULL;
  }
#endif /* CA_ROUTE_AES_CMAC */
}


/**
  * @brief   StSafeA_AES_ECB_Encrypt
  *          AES ECB Encryption
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pInData : plain data buffer
  * @param   pOutData : encrypted output data buffer
  * @param   InAesType : type of AES. Can be one of the following values:
  *            @arg STSAFEA_KEY_TYPE_AES_128: AES 128-bits
  *            @arg STSAFEA_KEY_TYPE_AES_256: AES 256-bits
  * @retval  0 if success, an error code otherwise
  */
int32_t StSafeA_AES_ECB_Encrypt(uint8_t *pInData, uint8_t *pOutData, uint8_t InAesType)
{
#if defined CA_ROUTE_AES_ECB && ((CA_ROUTE_AES_ECB & CA_ROUTE_AES_CFG_ENCRYPT_ENABLE) == CA_ROUTE_AES_CFG_ENCRYPT_ENABLE)
  int32_t status_code;
  CA_AESECBctx_stt aes_ctx;
  int32_t output_length = 0;

  switch (InAesType)
  {
    case STSAFEA_KEY_TYPE_AES_128:
    case STSAFEA_KEY_TYPE_AES_256:
      aes_ctx.mFlags = CA_E_SK_DEFAULT;
      aes_ctx.mKeySize = STSAFEA_AES_KEY_BITSIZE((uint32_t)InAesType) / 8U;
      status_code = CA_AES_ECB_Encrypt_Init(&aes_ctx, aHostCipherKey, NULL);
      if (status_code == CA_AES_SUCCESS)
      {
        status_code = CA_AES_ECB_Encrypt_Append(&aes_ctx, pInData, (int32_t)(STSAFEA_HOST_KEY_LENGTH / 8U), pOutData,
                                             &output_length);
        if (status_code == CA_AES_SUCCESS)
        {
          status_code = CA_AES_ECB_Encrypt_Finish(&aes_ctx, &pOutData[output_length], &output_length);
        }
      }
      break;

    default:
      status_code = 1;
      break;
  }

  return status_code;
#else
  return 1;
#endif /* CA_ROUTE_AES_ECB - CA_ROUTE_AES_CFG_ENCRYPT_ENABLE */
}


/**
  * @brief   StSafeA_AES_CBC_Encrypt
  *          AES CBC Encryption
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pInData : plain data buffer
  * @param   InDataLength : plain data buffer length
  * @param   pOutData : encrypted output data buffer
  * @param   InInitialValue : initial value
  * @param   InAesType : type of AES. Can be one of the following values:
  *            @arg STSAFEA_KEY_TYPE_AES_128: AES 128-bits
  *            @arg STSAFEA_KEY_TYPE_AES_256: AES 256-bits
  * @retval  0 if success, an error code otherwise
  */
int32_t StSafeA_AES_CBC_Encrypt(uint8_t *pInData, uint16_t InDataLength, uint8_t *pOutData,
                                uint8_t *InInitialValue, uint8_t InAesType)
{
#if defined CA_ROUTE_AES_CBC && ((CA_ROUTE_AES_CBC & CA_ROUTE_AES_CFG_ENCRYPT_ENABLE) == CA_ROUTE_AES_CFG_ENCRYPT_ENABLE)
  int32_t status_code;
  CA_AESCBCctx_stt aes_ctx;
  int32_t output_length = 0;

  switch (InAesType)
  {
    case STSAFEA_KEY_TYPE_AES_128:
    case STSAFEA_KEY_TYPE_AES_256:
      aes_ctx.mFlags = CA_E_SK_DEFAULT;
      aes_ctx.mKeySize = STSAFEA_AES_KEY_BITSIZE((uint32_t)InAesType) / 8U;
      aes_ctx.mIvSize = 16;
      status_code = CA_AES_CBC_Encrypt_Init(&aes_ctx, aHostCipherKey, InInitialValue);
      if (status_code == CA_AES_SUCCESS)
      {
        status_code = CA_AES_CBC_Encrypt_Append(&aes_ctx, pInData, (int32_t)InDataLength, pOutData, &output_length);
        if (status_code == CA_AES_SUCCESS)
        {
          status_code = CA_AES_CBC_Encrypt_Finish(&aes_ctx, &pOutData[output_length], &output_length);
        }
      }
      break;

    default:
      status_code = 1;
      break;
  }

  return status_code;
#else
  return 1;
#endif /* CA_ROUTE_AES_CBC - CA_ROUTE_AES_CFG_ENCRYPT_ENABLE */
}


/**
  * @brief   StSafeA_AES_CBC_Decrypt
  *          AES CBC Decryption
  * @note    This is a weak function that MUST be implemented at application interface level.
  *          A specific example template stsafea_crypto_xxx_interface_template.c is provided with this Middleware
  *
  * @param   pInData : encrypted data buffer
  * @param   InDataLength : encrypted data buffer length
  * @param   pOutData : plain output data buffer
  * @param   InInitialValue : initial value
  * @param   InAesType : type of AES. Can be one of the following values:
  *            @arg STSAFEA_KEY_TYPE_AES_128: AES 128-bits
  *            @arg STSAFEA_KEY_TYPE_AES_256: AES 256-bits
  * @retval  0 if success, an error code otherwise
  */
int32_t StSafeA_AES_CBC_Decrypt(uint8_t *pInData, uint16_t InDataLength, uint8_t *pOutData,
                                uint8_t *InInitialValue, uint8_t InAesType)
{
#if defined CA_ROUTE_AES_CBC && ((CA_ROUTE_AES_CBC & CA_ROUTE_AES_CFG_DECRYPT_ENABLE) == CA_ROUTE_AES_CFG_DECRYPT_ENABLE)
  int32_t status_code;
  CA_AESCBCctx_stt aes_ctx;
  int32_t output_length = 0;

  switch (InAesType)
  {
    case STSAFEA_KEY_TYPE_AES_128:
    case STSAFEA_KEY_TYPE_AES_256:
      aes_ctx.mFlags = CA_E_SK_DEFAULT;
      aes_ctx.mKeySize = STSAFEA_AES_KEY_BITSIZE((uint32_t)InAesType) / 8U;
      aes_ctx.mIvSize = 16;
      status_code = CA_AES_CBC_Decrypt_Init(&aes_ctx, aHostCipherKey, InInitialValue);
      if (status_code == CA_AES_SUCCESS)
      {
        status_code = CA_AES_CBC_Decrypt_Append(&aes_ctx, pInData, (int32_t)InDataLength, pOutData, &output_length);
        if (status_code == CA_AES_SUCCESS)
        {
          status_code = CA_AES_CBC_Decrypt_Finish(&aes_ctx, &pOutData[output_length], &output_length);
        }
      }
      break;

    default:
      status_code = 1;
      break;
  }

  return status_code;
#else
  return 1;
#endif /* CA_ROUTE_AES_CBC - CA_ROUTE_AES_CFG_DECRYPT_ENABLE */
}


/**
  * @}
  */
