...
/**
  * @brief Entropy callback for a safe hardware random generation.
  *
  */
int tkms_hardware_random(void *p_rng, unsigned char *output, size_t output_len)
{
  int ret = MBEDTLS_ERR_ECP_RANDOM_FAILED;
  (void) p_rng;
  size_t len = 0;

  ret = mbedtls_hardware_poll(NULL, output, output_len, &len);
  if (len == 0)
  {
    return ret;
  }
...
CK_RV tkms_app_ECDH_generate_derive_key(void)
{
  ...
  template_pub_key[0].pValue     = P11_OBJ_LABEL_EPH_PUB_KEY;
  template_pub_key[0].ulValueLen = sizeof(P11_OBJ_LABEL_EPH_PUB_KEY);
  ...
  (void)mbedtls_ecp_group_load(&peer_ctx.MBEDTLS_PRIVATE(grp), MBEDTLS_ECP_DP_SECP256R1);

  /* Import Device ephemeral public key generated with C_GenerateKeyPair */
  mbedtls_ecp_point_init(&dev_eph_pub_key_ec_point);
  (void)mbedtls_ecp_point_read_binary(&peer_ctx.MBEDTLS_PRIVATE(grp), &dev_eph_pub_key_ec_point, &a_DeviceEphemeralPubKeyEcPoint[2],
                                      DeviceEphemeralPubKeyEcPointLen - 2U);

  /* Import Peer Private key */
  (void)mbedtls_mpi_read_binary(&peer_ctx.MBEDTLS_PRIVATE(d), a_PeerPrvKey, PeerPrvKeyEcPointLen);
  (void)mbedtls_ecp_check_privkey(&peer_ctx.MBEDTLS_PRIVATE(grp), &peer_ctx.MBEDTLS_PRIVATE(d));
  ...
  (void)mbedtls_ecp_mul(&peer_ctx.MBEDTLS_PRIVATE(grp), &peer_shared_secret_x_ec_point, &peer_ctx.MBEDTLS_PRIVATE(d), &dev_eph_pub_key_ec_point, tkms_hardware_random,
                        NULL);

  (void)mbedtls_mpi_write_binary(&peer_shared_secret_x_ec_point.MBEDTLS_PRIVATE(X), a_PeerSecretX, PeerSecretXLen);
