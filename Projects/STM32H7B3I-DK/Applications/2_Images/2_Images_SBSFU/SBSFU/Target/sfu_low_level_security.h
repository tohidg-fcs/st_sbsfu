...
#define SFU_SEC_USER_MEM_END_FROM_OB ((uint32_t)((SFU_SEC_MEM_AREA_ADDR_END>>8U)<<8U))
/*!< Bank1 Secure User memory size */
...
#define SFU_PROTECT_WRP_PAGE_START_1    ((uint32_t)((SFU_BOOT_BASE_ADDR - FLASH_BASE) / (FLASH_SECTOR_SIZE*4U)))

/*!< Last sector: (code_size-1)/sector_size because the
     page indexes start from 0 */
#define SFU_PROTECT_WRP_PAGE_END_1      ((uint32_t)((SFU_ROM_ADDR_END - FLASH_BASE) / (FLASH_SECTOR_SIZE*4U)))

#if (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER)
/*!< First sector of LOADER ROM area */
#define SFU_PROTECT_WRP_LOADER_START_1    ((uint32_t)((LOADER_REGION_ROM_START - FLASH_BASE) / (FLASH_SECTOR_SIZE*4U)))

/*!< Last sector of LOADER ROM area */
#define SFU_PROTECT_WRP_LOADER_END_1      ((uint32_t)((LOADER_REGION_ROM_END - FLASH_BASE) / (FLASH_SECTOR_SIZE*4U)))
...
/**
  * @brief Region 0 - Enable the read/write operations for the all peripheral in unprivileged mode.
...
#define SFU_PROTECT_MPU_FLASHACC_START CD_AXIFLASH_BASE    /*!< FLASH memory area */
...
#define SFU_PROTECT_MPU_FLASHEXE_START CD_AXIFLASH_BASE    /*!< FLASH memory area */
...
#endif /* (__GNUC__) || (__CC_ARM) || (__ARMCC_VERSION) */
#if defined (__GNUC__) || defined(__CC_ARM) || defined(__ARMCC_VERSION)
#define SFU_PROTECT_MPU_FLASHEXE_SREG  0xC0U               /*!< 128 Kbytes / 8 * 6 ==> 96 Kbytes */
#else
...
#define SFU_PROTECT_MPU_SRAMACC_START CD_DTCMRAM_BASE      /*!< RAM memory area */
...
#define SFU_PROTECT_MPU_EXEC_SE_START CD_AXIFLASH_BASE    /*!< FLASH memory area */
...
#define RTC_TAMPER_ID_INTERRUPT            RTC_IT_TAMP1
