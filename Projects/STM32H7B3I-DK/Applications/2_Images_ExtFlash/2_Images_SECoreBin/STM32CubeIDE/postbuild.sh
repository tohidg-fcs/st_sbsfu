...
magic="SFU"$fwid
oemkey=$SecureEngine"/../Binary/OEM_KEY_COMPANY"$fwid"_key_AES_CTR.bin"
...
if [ $ret -eq 0 ]; then
  ...
  ret=$?
  if [ $ret -eq 0 ]; then 
    command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" -r 36 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" "$sfb" -o "$offset
    $command >> $projectdir"/output.txt"
    ret=$?
    if [ $ret -eq 0 ]; then
      command=$cmd" "$prepareimage" header -m "$magic" -k  "$ecckey" -r 36 -v "$version"  -i "$nonce" -f "$sfu" -t "$sign" -o "$offset" "$headerbin
      $command >> $projectdir"/output.txt"
      ret=$?
      if [ $ret -eq 0 ]; then
          ...
          if [ $ret -eq 0 ] && [ -e "$ref_userapp" ]; then
            ...
            if [ $ret -eq 0 ]; then
              ...
              if [ $ret -eq 0 ]; then
                ...
                ret=$?
                if [ $ret -eq 0 ]; then
                  command=$cmd" "$prepareimage" pack -m "$magic" -k "$ecckey" -r 36 -v "$version" -n "$nonce" -f "$sfu" -t "$sign" -o "$offset" --pfw "$partialsfu" --ptag "$partialsign" --poffset  "$partialoffset" "$partialsfb
                  $command >> $projectdir"/output.txt"
                  ret=$?
                fi
              fi
