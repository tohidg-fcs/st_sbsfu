...
set "sbsfuelf=%SBSFUBootLoader%\\2_Images_SBSFU\\EWARM\\STM32H7B3I_DISCO\\Exe\\STM32H7B3I_DISCO.out"
...
set "command=%python%%prepareimage% merge -v 0  -i %headerbin% -s %sbsfuelf% -x %header% %bigbinary% >> %projectdir%\output.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
