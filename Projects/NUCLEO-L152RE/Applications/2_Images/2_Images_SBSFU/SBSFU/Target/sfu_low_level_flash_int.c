...
SFU_ErrorStatus SFU_LL_FLASH_INT_Erase_Size(SFU_FLASH_StatusTypeDef *pFlashStatus, uint8_t *pStart, uint32_t Length)
{
  ...
  {
    if (((uint32_t)pStart >= SlotHeaderAdd[SLOT_ACTIVE_1 + i]) &&
        (((uint32_t)pStart + length) <= (SlotHeaderAdd[SLOT_ACTIVE_1 + i] + SFU_IMG_IMAGE_OFFSET)))
    {
      inside_header = 1U;
    }
  }
  ...
  /* Destination area part of 1 of the firmware image headers :
     erasing operation should be executed inside secure environment */
...
uint32_t SFU_LL_FLASH_INT_GetSector(uint32_t Add)
{
  ...
  sector = (Add - FLASH_BASE) / (FLASH_PAGE_SIZE * 16U);
...
static SFU_ErrorStatus SFU_LL_FLASH_INT_Clear_Error(void)
{
  ...
  if (HAL_FLASH_Unlock() == HAL_OK)
  {
    ...
    __HAL_FLASH_CLEAR_FLAG(FLASH_FLAG_EOP     | FLASH_FLAG_PGAERR   | FLASH_FLAG_SIZERR |
                           FLASH_FLAG_OPTVERR | FLASH_SR_OPTVERRUSR | FLASH_FLAG_WRPERR);
