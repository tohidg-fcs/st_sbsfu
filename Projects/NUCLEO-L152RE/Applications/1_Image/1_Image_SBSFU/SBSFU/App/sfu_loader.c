...
static uint32_t m_uActiveHeaderStart = 0U;                       /*!< Address of the active header (in 1_Image example: dwl_slot == active_slot
                                                                      but the active header may be not contiguous) */
static uint32_t m_uActiveHeaderAddress = 0U;                     /*!< Address of the write in the active header area */
...
SFU_ErrorStatus SFU_COM_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  if (m_uPacketsReceived == 1U)
  {
    ...
    /*
     *  On this series, the erasing process will write 0X00000000 on the beginning of the page to be erased
     *  ==> as the header in a 1 slot per image example is in the isolated area, this action should be done by
     *      Secure Engine
     *  Thus the the erasing is split in 2 actions (HEADER + FW)
     */
    /* Clear header */
    if (e_ret_status == SFU_SUCCESS)
    {
      if (SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *) m_uActiveHeaderAddress, SFU_IMG_IMAGE_OFFSET) != SFU_SUCCESS)
      {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      if (SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *)(m_uDwlAreaAddress + SFU_IMG_IMAGE_OFFSET),
                                  SLOT_SIZE(m_uDwlSlot) - SFU_IMG_IMAGE_OFFSET) != SFU_SUCCESS)
      {
        m_LoaderStatus = SFU_LOADER_ERR_FLASH;
        e_ret_status = SFU_ERROR;
      }
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if ((rx_size % (uint32_t)sizeof(SFU_LL_FLASH_write_t)) != 0U)
    {
      ...
      {
        fw_decrypted_chunk[uOldSize] = 0x00;
        uOldSize++;
      }
    }
  }
...
SFU_ErrorStatus SFU_COM_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  else if (2U == m_uPacketsReceived)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      /* Clear header */
      if ((e_ret_status == SFU_SUCCESS)
          && (SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *) m_uActiveHeaderAddress, SFU_IMG_IMAGE_OFFSET)
              != SFU_SUCCESS))
      {
        m_LoaderStatus = SFU_LOADER_ERR_FLASH;
        e_ret_status = SFU_ERROR;
      ...
      /* Clear Download application area (including TRAILERS area) */
      if ((e_ret_status == SFU_SUCCESS)
          && (SFU_LL_FLASH_Erase_Size(&x_flash_info, (uint8_t *)(m_uDwlAreaAddress + SFU_IMG_IMAGE_OFFSET),
                                      SLOT_SIZE(m_uDwlSlot) - SFU_IMG_IMAGE_OFFSET) != SFU_SUCCESS))
      {
        m_LoaderStatus = SFU_LOADER_ERR_FLASH;
        e_ret_status = SFU_ERROR;
      ...
      if (e_ret_status == SFU_SUCCESS)
      {
        ...
        else
        {
          ...
          if (SFU_LL_FLASH_Write(&x_flash_info, (uint8_t *)m_uActiveHeaderAddress, fw_header, SE_FW_HEADER_TOT_LEN)
              == SFU_SUCCESS)
          {
            /* Shift the DWL area pointer */
            m_uDwlAreaAddress += SE_FW_HEADER_TOT_LEN;
