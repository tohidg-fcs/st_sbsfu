...
@par Directory contents
   ...
   - 2_Images_UserApp/Src/stm32wbxx_it.c             Interrupt handlers
   - 2_Images_UserApp/Src/system_stm32wbxx.c         STM32 system file
   ...
   - 2_Images_UserApp/Inc/stm32wbxx_conf.h           HAL configuration file
   - 2_Images_UserApp/Inc/stm32wbxx_it.h             Header for stm32wbxx_it.c file
...
In this example, the user application relies on BLE_OTA project for the download functionality.
As example, STM32CubeMonitor-RF can be used on host side to send the user encrypted firmware file (.sfb).
...
2. Pressing 2 allows to test protections.
   - CORRUPT IMAGE test (#1): not possible to corrupt external FLASH during UserApp execution. Write and erase flash
     commands are not allowed in memory map mode configuration of external flash.
   ...
   - TAMPER test (#4): causes a reset if a tamper event is detected. In order to generate a tamper event, user has to
     connect PA0 (CN8.4) to GND (It may be enough to put your finger close to PA0 (CN8.4)). It may be better to
     have PA0 (CN8.4) connected to 3V3 before doing the transition to GND.
...
Feature not supported with standalone loader configuration.
