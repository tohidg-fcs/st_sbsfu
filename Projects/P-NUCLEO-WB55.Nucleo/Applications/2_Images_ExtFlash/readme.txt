...
The Secure Firmware Update application receives the firmware image via BLE protocol, checks its authenticity, and checks
the integrity of the code before installing it. The firmware update is done on the complete firmware image, or only on a
...
This example is ready for an external flash configuration with backup slot mechanism. When available on board:
   - Download slot should be mapped in external flash
   - Backup slot should be mapped in external flash
   - Active slot should be extended to all the internal flash free
   - External flash driver should be added in sfu_low_level_flash_ext.c
   
@par Directory contents

   - BLE_Ota             BLE Ota loader application
...
0. Compile BLE_Ota application
   The BLE_Ota binary is linked with the SBSFU code in step 2. 
