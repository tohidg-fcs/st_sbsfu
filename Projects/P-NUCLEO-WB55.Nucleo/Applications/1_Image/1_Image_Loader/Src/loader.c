...
static uint32_t m_uDwlSlot = SLOT_ACTIVE_1;                      /*!< 1 slot per image config : active slot also used for download */
...
/**
  ...
  * @param  pDwSlot: Identification of the downloaded area.
...
static LOADER_StatusTypeDef LOADER_VerifyFwHeader(uint8_t *pBuffer)
{
  ...
  if (slot_number != SLOT_INACTIVE)
  {
    ...
    if (p_header->FwSize  > (SLOT_SIZE(m_uDwlSlot) - IMAGE_OFFSET))
    {
  ...
  {
    /*
     * Magic SFU1/SFU2/SFU3 not identified :
     *   - we are downloading a wireless stack or FUS
     *   - download area should be SLOT_ACTIVE_1
     *   - after CM0 FUS/Stack update we will reinstall UserApp from BACKUP slot
     */
    m_uDwlType = DWL_OTHER;
    m_uDwlSlot = SLOT_ACTIVE_1;
    m_uDwlAreaStart =  SlotStartAdd[m_uDwlSlot];
    m_uDwlAreaSize  =  SLOT_SIZE(m_uDwlSlot);
    e_ret_status = LOADER_OK;
...
static LOADER_StatusTypeDef LOADER_YMODEM_DataPktRxCpltCallback(uint8_t *pData, uint32_t uSize)
{
  ...
  else if (2 == m_uPacketsReceived)
  {
    ...
    if (e_ret_status == LOADER_OK)
    {
      ...
      m_uDwlImgSize = ((SE_FwRawHeaderTypeDef *)(uint32_t)fw_header)->FwSize + IMAGE_OFFSET;
